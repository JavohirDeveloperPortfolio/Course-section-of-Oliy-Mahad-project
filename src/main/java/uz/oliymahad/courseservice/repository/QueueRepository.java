...
public interface QueueRepository extends JpaRepository<QueueEntity,Long> {
    ...
    @Query(value = "select userid from (select *,RANK() over(order by applieddate asc) from queueentity where course_id = :courseId and status = :status and gender = :gender ) as sub  limit :limit", nativeQuery = true)
